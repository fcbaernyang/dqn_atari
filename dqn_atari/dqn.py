...
class QLearner(object):
  ...
  def __init__(
    ...
    self.test = test
    if not test:
      logz.configure_output_dir('exp_data/')
    ###############
    # BUILD MODEL #
    ###############
    ...
    with tf.device('/device:GPU:0'):
      ...
      idx = tf.transpose(tf.stack([tf.range(N), self.act_t_ph]))
      self.act = tf.squeeze(tf.argmax(Q, axis = 1))
      v1 = tf.gather_nd(Q, idx)
      v2 = tf.reduce_max(target_Q, axis = 1)
      self.total_error = 0.5*tf.reduce_sum(huber_loss(v2 * gamma * (1 - self.done_mask_ph) + self.rew_t_ph - v1))
      q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
  ...
  def step_env(self):
    ...
    #####
    if self.t == 0 and not self.test:
      self.session.run(tf.global_variables_initializer())
    idx = self.replay_buffer.store_frame(self.last_obs)
    if self.test:
      self.env.render()
      time.sleep(0.016)
    if self.test or np.random.uniform()>self.exploration.value(self.t):
      recent_ob = self.replay_buffer.encode_recent_observation()
      action = self.session.run(self.act, feed_dict={self.obs_t_ph: [recent_ob]})
    ...
    if done:
      _ = self.replay_buffer.store_frame(self.last_obs)
      #self.evn.seed(random.randint(0, 9999))
      self.last_obs = self.env.reset()
      #if self.test:
      #  self.test_ob = self.env_test.reset()
  ...
  def update_model(self):
    ...
    if (self.t > self.learning_starts and \
      ...
      obs_batch, act_batch, rew_batch, next_obs_batch, done_mask = self.replay_buffer.sample(64)
      ...
      self.loss, _ = self.session.run([self.total_error, self.train_fn], feed_dict={self.obs_t_ph: obs_batch, self.act_t_ph: act_batch,
                                                 self.rew_t_ph: rew_batch, self.obs_tp1_ph: next_obs_batch,
                                                 self.done_mask_ph: done_mask, self.learning_rate: self.optimizer_spec.lr_schedule.value(self.t)})
  ...
  def log_progress(self):
    ...
    if self.t % self.log_every_n_steps == 0 and self.model_initialized:
      ...
      #if self.start_time is not None:
      #  logz.log_tabular("Running Time", (time.time() - self.start_time) / 60.)
      logz.log_tabular("Episodes", len(episode_rewards))
      logz.log_tabular("Value_Loss", self.loss)
      logz.log_tabular("AverageReturn", self.mean_episode_reward)
      logz.log_tabular("MaxAvgReturn", self.best_mean_episode_reward)
...
def learn(*args, **kwargs):
  ...
  if not alg.test:
    while alg.t < alg.stop_step:
      ...
      alg.update_model()
      alg.log_progress()
    saver.save(alg.session,'AtariModel_Pong')
  else:
    saver.restore(alg.session, 'AtariModel_Pong')
